...
Thin HTML files (~120–150 lines). Content order (v1.1):
...
10. Five `<script>` tags in load order (see below)
...
## Dashboard Section Order (v1.1)

The combined accordion and AI panel are **first** on every asset page. Individual TF tables are below the divider as supporting reference detail. TradingView chart is last (above the footnote).

```
Header + Legend (toggle)
[TAB BAR: Seasonals · Chart · Analysis]         ← injected by ui.js
  [SUB-TAB BAR: Combined · 5-YR · 15-YR · LT]  ← injected by ui.js, only on Seasonals tab
...
- Script src tags in correct order: `data/[asset].js` → `accordion.js` → `api.js` → `tradingview.js` → `ui.js`
