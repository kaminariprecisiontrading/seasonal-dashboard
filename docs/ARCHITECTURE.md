...
- **`profiling-profiles/detail.html`** (`?p=<slug>&a=<assetkey>`) — one templated page for all 8 profiles (not 8 files), showing the rule/why/timing copy, cross-asset stats, and a real illustrative M15 candlestick example day per asset. Controller: `js/profiling-profile-detail.js`. Unlike the source (which had to reference bare `const` identifiers because its data files weren't `window`-namespaced — see `market-profiling-system-spec.md` §5.3), this port's data already resolves via `window.KPT_PROFILING`/`window.KPT_PROFILING_EXAMPLES`, so that workaround isn't needed.
...
| Profiling calendar/profile-detail page blank or console error | `profiling-calendar/index.html` and `profiling-profiles/detail.html` each load `js/profiling-charts.js` directly (they're outside the tab system, not `ASSET_CONFIG`-dependent) — check that tag is present; the profile-detail page also needs both assets' `data/profiling/{gbpusd,eurusd}.js` and `data/profiling/profile-examples/*.js` loaded |
